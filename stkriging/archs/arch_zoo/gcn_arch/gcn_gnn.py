...
class D_GCN(nn.Module):
    ...
    def forward(self, X, A_q, A_h):
        ...
        if len(A_q.shape) == 2:
            ...
            x0 = X
            x0 = x0.permute(0, 2, 1)
            x = torch.unsqueeze(x0, 3)

            for support in supports:
                #print(x0.shape,support.shape)
                x1 = torch.bmm(x0,support)
                x = torch.cat([x,torch.unsqueeze(x1, 3)],dim=3)
                for k in range(2, self.orders + 1):
                    x2 = 2 *  torch.bmm(x1,support) - x0
                    x = torch.cat([x,torch.unsqueeze(x2, 3)],dim=3)
                    x1, x0 = x2, x1
            x = x.permute(0, 2, 1, 3)
