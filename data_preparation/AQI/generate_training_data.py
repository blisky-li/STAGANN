...
def generate_data(args: argparse.Namespace):
    ...
    Default settings of AQI dataset:
        ...
        - Channels (features): three channels [air quality, time of day, day of week]
        - Target: predict the air quality of the unknown nodes.
    ...
    seqlen = args.seq_len
    # read data
    #loc = pd.read_excel(graph_file_path)
    '''df = pd.read_excel(data_file_path, skiprows=0)
    data = df.values
    np.save('AQI.npy', data)'''
    #df = pd.read_excel(data_file_path)
    #df = pd.read_excel(data_file_path , sheet_name=0, index_col=0)
    df = np.load(data_file_path)


    data = np.expand_dims(df, axis=-1).astype(float)
    print(data[:,0,:])
    print(data.shape)
    ...
    loc = pd.read_excel(graph_file_path).values
    loc = np.array(loc)
    print(loc.shape)
    '''location = {}
    location["lon_lat"] = loc
    ...
    for i in range(N):
        for j in range(N):
            lat1, lon1 = map(float, loc[i])
            lat2, lon2 = map(float, loc[j])
            dist_matrix[i, j] = haversine(lat1, lon1, lat2, lon2)
    sigma = dist_matrix.std()
    adj = np.exp(-np.square(dist_matrix / sigma))
    adj[adj < 0.1] = 0.
    print(adj.shape,'sssssssssssssssssssssssssssssss')
    dic_adj = {}
    dic_adj['adj_mx'] = adj
    print(adj.shape)
    print(processed_data.dtype, adj.dtype)
...
if __name__ == "__main__":
    ...
    DATA_FILE_PATH = ORIGIN_DIR + "/datasets/raw_data/{0}/{1}.npy".format(DATASET_NAME, DATASET_NAME2)
    GRAPH_FILE_PATH = ORIGIN_DIR + "/datasets/raw_data/{0}/data.xlsx".format(DATASET_NAME, DATASET_NAME2)
